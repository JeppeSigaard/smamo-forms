...
function smamo_rest_form($data){
    ...
    /* felter */
    $response['fields'] = get_post_meta($form_id,'form_fields', true);
    
    /* logik */
    $response['logic_active'] = get_post_meta($form_id,'logic_active', true);
    if('checked' === $response['logic_active']){
        $response['logic'] = get_post_meta($form_id,'logic', true);
    }
